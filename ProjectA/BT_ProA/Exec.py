...
from BT_ProA.Node import Node
from BT_ProA.Search import search
from BT_ProA.Stn import stnCreate, stnLookUp
...
def exec_(num_of_planes, num_of_lanes, planes, state, chosen_lanes, is_gui, config_name, log_output,
    ...
    # init is the first node of the problem
    ...
    if not is_gui:
        print("\n\n=====================================================================", file=sys.stdout)
        print('                            ', colored(config_name, attrs=['bold']), file=sys.stdout)
        print("=====================================================================", file=sys.stdout)
    if search_return_val.valid:  # solution found
        print("solution found :)", file=sys.stdout)
        max_time = max(r[1] for r in search_return_val.chosen_var_list)
        print("max time: ", max_time, file=sys.stdout)
        ...
        print("can't find solution :(", file=sys.stdout)
    if not is_gui:
        print("exec order (as we target): ", file=sys.stdout)
        print([str(r[0]) + "_" + str(r[2]) for r in sort_planes], file=sys.stdout)
    if len(all_chosen_var_list) > 0:
        ...
        print("exec order (by the algorithm): ", file=sys.stdout)
        print([str(r[0]) + "_" + str(r[1]) for r in exec_order], file=sys.stdout)
        print("=====================================================================", file=sys.stdout)
        if is_print_full_solution:
            print("=====================================================================", file=sys.stdout)
            print("final solution: ", file=sys.stdout)
            print("=====================================================================", file=sys.stdout)
            for i in range(len(final_lane_list)):
                print("plane", i, ":  [take off lane id, landing lane id] = ",
                      final_lane_list[i], file=sys.stdout)
            print("=====================================================================", file=sys.stdout)
    ...
    if is_print_full_solution:
        print("=====================================================================", file=sys.stdout)
        print("total run time: ", timeit.default_timer() - first_start_time, file=sys.stdout)
        print("=====================================================================", file=sys.stdout)
